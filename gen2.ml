...
let translate (globals, functions) =
    ...
    let codegen_func func_decl =
        ...
        let rec add_local map (vdecl: A.vardecl) =
            let typ = vdecl.declTyp in
            let name = vdecl.declID in
        ...
        let local_vars =
            ...
            in
            List.fold_left2 add_formal StringMap.empty func_decl.A.args (Array.to_list (L.params f))
            (* List.fold_left add_local formals (List.map translate_stmt func_decl.body) *)
        in

        (* see if its in the local_vars map before something's used.. should be ok via semantic analysis *)
       ...
       (* TODO: Unop, Asn, ArrayListInit, CanvasInit, Noexpr *) 
        ...
        (* handle variable declarations: int a; int b = 5; etc.. *)
        and codegen_vdecl (vdecl: A.vardecl) llbuilder =
            let typ = (ltype_of_typ vdecl.declTyp) in
            let name = vdecl.declID in

            (* add the name to the local_vars map *)
            add_local local_vars vdecl in

            let init_expr = vdecl.declInit in
                match init_expr with 
                  A.Noexpr ->  llbuilder
                | e        -> (codegen_asn name e llbuilder); llbuilder
